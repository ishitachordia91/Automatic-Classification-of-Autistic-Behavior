...
f2345=[];
for t=6:10:size(w0,1)       %%start, stop, steps
    ...
    % clear meanRight;clear meanLeft;clear meanTorso;
                    meanRight=mean(windowedDataRight);meanLeft=mean(windowedDataLeft);meanTorso=mean(windowedDataTorso); %%gives mean of each column
